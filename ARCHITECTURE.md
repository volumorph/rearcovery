...
- Проверка пользователем: «О приложении» → SHA-256 на экране == хэш в релизе/README. Для локальной (file://) копии это работает потому, что сборка сериализационно-стабильна: `build.mjs` на этапе сборки линтит шаблон (запрещены: пробелы между `<html>`/`<head>` и `</body>`/`</html>`, хвостовые пробелы после `</html>`, минимизированные булевы атрибуты — только `checked=""`, одинарные кавычки атрибутов — только двойные, сырые `&` вне сущностей) и при нарушении останавливает сборку. `boot()` захватывает `APP_SOURCE` до `renderModeBadge()` и любых других мутаций DOM.
